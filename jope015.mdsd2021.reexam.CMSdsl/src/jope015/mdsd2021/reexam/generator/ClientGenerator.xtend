...
class ClientGenerator {
	...
	val path = "client/"
		...
		def generateClient(Project project, IFileSystemAccess2 fsa) {
			...
			entities.forEach[ e |
				...
				fsa.generateFile(path + 'src/components/' + e.name.toFirstUpper +'Display.jsx', e.compileDisplayEntityComponent)
	...
	def compileService(Entity e)
		...
		export const get«e.name.toFirstUpper»s = () => axios.get(`http://localhost:5000/«e.name.toFirstLower»s`)
		  .then(res => res.data)
		  .catch(err => console.log('error ocurred when making the request: ', err))
	...
	def compileResourceTable()
		...
		import { Label, Menu, Table, Icon, Button } from 'semantic-ui-react'
		import { Link } from 'react-router-dom'
		import styled from 'styled-components'
		...
		const ResourceTable = ({ resource, name }) => {
		  ...
		  console.log('schema: ', resourceSchema)
		  ...
		  return (
		    <React.Fragment>  
			    ...
			    <Table.Header>
			      <Table.Row>
			        {resourceSchema !== undefined && getKeys(resourceSchema).map(key => (<Table.HeaderCell>{key}</Table.HeaderCell>))}
			  ...
			  useEffect(() => {
			    get«e.name.toFirstUpper»s().then(res => {
					...
					«ENDIF»
			      	...
			      	set«e.name.toFirstUpper»List(res)
			    })
		...
		def compileForm(Entity e)
			...
			const Create«e.name.toFirstUpper»Form = () => {
			  ...
			  const handleSubmit = (e) => {
			    e.preventDefault()
			    create«e.name.toFirstUpper»(formData).then(res => {
...
//			}
		...
		def dispatch compileInputFieldFromDataType(Str type, Field f) '''<Form.Input type='text' name="«f.name»" onChange={handleChange} placeholder='«f.name»' />'''
		def dispatch compileInputFieldFromDataType(Integ type, Field f) '''<Form.Input type='number' name="«f.name»" onChange={handleChange} placeholder='«f.name»' />'''
		def dispatch compileInputFieldFromDataType(Lng type, Field f) '''<Form.Input type='number' name="«f.name»" onChange={handleChange} placeholder='«f.name»' />'''
		def dispatch compileInputFieldFromDataType(Flt type, Field f) '''<Form.Input type='number' name="«f.name»" onChange={handleChange} placeholder='«f.name»' />'''
		def dispatch compileInputFieldFromDataType(Dt type, Field f) '''<Form.Input type='date' name="«f.name»" onChange={handleChange} placeholder='«f.name»' />'''
